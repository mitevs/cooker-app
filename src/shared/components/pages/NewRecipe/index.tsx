...
const NewRecipe = () => {
        ...
        try {
            await createRecipe({
                ...
                update: (store, { data: { createRecipe } }) => {
                    const data: any = store.readQuery({ query: GET_RECIPES, variables: { authorId: user.id } });
                    data.recipes = [...data.recipes, { ...newRecipe, createRecipe }];
                    store.writeQuery({ query: GET_RECIPES, data, variables: { authorId: user.id } });
                }
            });
    ...
    return <SingleColumn>
        ...
        <form onSubmit={onSubmit} noValidate>
            ...
            <FormControl label="Nutrition Facts"
                name="nutritionFacts"
                onChange={onChange}
            ...
            <FormControl label="Preparation Steps"
                name="prepSteps"
                onChange={onChange}
